...
export default function Dropdown({data, placeholder, callback}) {
    const handler = (e) => {
        ...
        const obj = data.filter((item,index)=> item.id == val)
        callback(obj[0])
    }
    return (
        <div className="w-full flex flex-wrap items-center relative ">
            <select onChange={handler} className="w-full text-sm py-3 pl-3 pr-2 cursor-pointer border rounded-lg focus:outline-none focus:border-2 focus:border-neutral4">
                <option  className="text-sm">{placeholder}</option>
                {data && data.map((item,index)=> <option className="text-sm p-2 cursor-pointer" key={`${placeholder}-${index}`} value={item.id}>{item.value}</option>)}
            </select>
